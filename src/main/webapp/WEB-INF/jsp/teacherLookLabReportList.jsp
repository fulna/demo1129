...
<!-- <div id="div11" style="width:100%; height:20px;padding-left:40%;">
	...
	<script type="text/javascript">
	 $(function(){
		        ...
		        columns: [
		                  ...
		                  { data: 'grade' },
 		                  { data: null,"render":function(data,type,row,meta){
		                	  var html="<button   id='toLookReport' name='toLookReport' class='btn btn-info btn-xs'  data-toggle='modal' data-target='#myModalReport' style='margin:0px;margin-right:3px;'  >查看</button><button id='toShareReport' name='toShareReport' data-toggle='modal' data-target='#myModalSharePeople' class='btn btn-danger btn-xs' style='margin:0px;margin-right:3px;'>分享</button><button id='toExportReport' name='toExportReport' class='btn btn-success btn-xs' style='margin:0px;'>导出</button>";
		                  	  return html;
 		                  	}
		                  }
		...
		$(document).on("click", "#toShareReport", function(se) {  //存储Rid到session
			var $select = $(this).parent().parent();
			var rData = $('#tableData').DataTable().row($select).data();		
